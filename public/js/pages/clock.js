            ...
            function frame(){
                if(width <= 0){
                    ...
                    if(user.email) { 
                        ...
                        if(JSON.parse(localStorage.getItem('banklogs')).length == 1) {
                            var msg = `15 Seconds Left! Anonymous User, <hr class="to-hr"> Let the timer run out, and then choose another banklog and pay for it on time `; 
                        } else {
                            var msg = `15 Seconds Left! Anonymous User, <hr class="to-hr"> Let the timer run out, and then choose another banklog and pay for it on time `;
                        }
                    }
                    ...
                    if(user.email) { 
                        ...
                        if(JSON.parse(localStorage.getItem('banklogs')).length == 1) {
                            ...
                            <hr class="to-hr"> ${(JSON.parse(localStorage.getItem('banklogs'))[0].account).replace(']',' ACCOUNT]')} before time runs out. <hr>Email invoice is a better option`; 
                        } else {
                            var msg = `1 Minute Left! <hr> Complete the $${coastDis} bitcoin payment to download: 
                            <hr class="to-hr"> ${(JSON.parse(localStorage.getItem('banklogs')).length)} bank logs before time runs out. <hr>Email invoice is a better option`; 
                        }
                    }
                    toastr.options = {closeButton: true, debug: false, newestOnTop: true, progressBar: true, positionClass: 'toast-top-full-width', preventDuplicates: true, onclick: null};
                    var $toast = toastr[shortCutFunction](msg);
