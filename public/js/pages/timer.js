...
if((user.isAnonymous && !localStorage.getItem('money-time')) || user.email || user.phoneNumber) {
        ...
        if((JSON.parse(localStorage.getItem('banklogs')).length) > 0) {
            ...
            function frame(){
                if(width <= 0){
                    clearInterval(id);
                    i = false;
                    localStorage.setItem('money-time', true);
                    ...
                    if(user.email) { 
                        ...
                        var msg = `15 Seconds Left!, <hr class="to-hr"> Let the timer run out. You'll be required to get an email / phone invoice next time `;
                    }
