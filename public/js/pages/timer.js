...
if((user.isAnonymous && !localStorage.getItem('vax-time')) || user.email || user.phoneNumber) {
        ...
        if((JSON.parse(localStorage.getItem('banklogs')).length) > 0) {
            ...
            function frame(){
                if(width <= 0){
                    clearInterval(id);
                    i = false;
                    localStorage.setItem('vax-time', true);
                    ...
                    if(user.email) { 
                        ...
                        var msg = `15 Seconds Left! Anonymous User, <hr class="to-hr"> Let the timer run out. <br> You'll be required to get an email / phone invoice next time`;
                    }
